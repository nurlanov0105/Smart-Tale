...
import {contactsData, ORDER_FORM_NAMES, sizesData} from "@/features/user/orderForm/model/consts.data";
import {
    descriptionSchema,
    priceSchema,
    titleSchema,
...
const AnnouncementDetailForm:FC<{type: string}> = ({type}) => {
    ...
    const {
        ...
        errors,
        isValid
    } = useAnnouncementDetail({type, images, deadline}) //Тип создания(заказа или оборудования)

    const isDisabled = () => {
        if (type === "order"){
            return !images.length || !sizesDate.length || day.postValue === 0 || month.postValue === 0 || year.postValue === 0
        }else {
        ...
        <form onSubmit={handleSubmit} className={clsx(styles.form, styles[theme])}>
            <div className={styles.order}>
                ...
                <div className={clsx(styles.order__block, styles.order__block_gap)}>
                    ...
                    />
                    <InputField
                        {...register(selectedContact.postValue as keyof OrderCreateFormType, typeSchema(selectedContact.postValue))}
