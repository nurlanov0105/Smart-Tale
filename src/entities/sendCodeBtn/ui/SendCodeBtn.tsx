...
const SendCodeBtn: FC<SendBtnProps> = ({isError, setIsError}) => {
    ...
    return (
        <>
            {
                isError ? <Button type="submit" disabled={isDisabled} onClick={handleSendAgain}>Отправить код еще раз</Button>
                    : <Button type="submit" onClick={handleSend}>Войти</Button>
            }
            {
                isDisabled ? <p className={styles.auth__button}>Отправить код повторно через 00:{secondsFormat}</p>
