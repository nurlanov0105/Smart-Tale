...
const CardsSection2: FC<IProps> = ({ initialData, queryKey, dependencies }) => {
   const { isFetchingNextPage, observerTarget, data, isLoading, isError } = useInfinityScroll2({
      initialData,
      queryKey,
      dependencies,
   });
   ...
   const readyData = () => {
      if (isError) {
         ...
         return [...Array(8)].map((_, i: number) => (
            <CommonSkeleton key={i} type={SkeletonTypes.standart} />
         ));
      } else if (!isFetchingNextPage && data?.length === 0) {
