...
export const OrganizationService = {
   createOrganization: async (params: FormData) => {
    ...
    deleteEmployee: async (employeeSlug: string) => {
        const response = await baseApiInstance.delete(OrganizationEndpoints.DELETE_EMPLOYEE + employeeSlug);
        return response.data;
    },
    ...
    activateOrganization: async (slug: string) => {
        const response = await baseApiInstance.put(OrganizationEndpoints.ORGANIZATION_ACTIVATE + slug);
        return response.data;
    },
